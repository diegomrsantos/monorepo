...
[build_backends.py](build_backends.py) builds the unchanged backend and two
...
16 MiB of input. The [main plan](plans/backend-grid.json) contains all 32 cases.
The [smoke plan](plans/backend-smoke.json) covers the same cases with shorter
runs. The [baseline pilot](plans/backend-aa.json) compares the identical
...
[check_archives.py](check_archives.py) audits all nine retained datasets, covering
...
[analyze_native.py](analyze_native.py) reports geometric means of paired ratios,
...
[render_native.py](render_native.py) regenerates the complete grid's table and
